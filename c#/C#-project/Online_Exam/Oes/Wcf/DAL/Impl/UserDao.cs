...
namespace WCF.DAL
{
     ...
     public class UserDao : IUserDao
    {
        /// <see cref="IUserDao.FindUserByUsername"/>
                 ...
                 using (SqlCommand command = new SqlCommand(Constants.FindUserByName, conn))
                 {
                     ...
                     using (SqlDataReader reader = command.ExecuteReader())
                     {
                         ...
                         if (reader.Read())
                         {
                             ...
                             string a = reader.GetString(13);
                             user.IsLogined = Constants.ZeroString.Equals(reader.GetString(13)) ? false : true;
                         }
                         else
                         {
                             user = null;
         ...
         public void UpdateLastLoginTime(int userId, DateTime loginTime, Boolean isLogined)
         {
             string constr = ConfigurationManager.AppSettings[Constants.SqlServer].ToString();
             using (SqlConnection conn = new SqlConnection(constr))
             {
                 ...
                 using (SqlCommand command = new SqlCommand(Constants.UpdateLastLoginTime, conn))
                 {
                     ...
                     string userIsLogined = isLogined == true ? Constants.OneString : Constants.ZeroString;
                     command.Parameters.Add(new SqlParameter(Constants.IsLoginedParameter, userIsLogined));
                     command.ExecuteNonQuery();
                 }
             }
         }

         /// <see cref="IUserDao.FindUserRoleByUserId"/>
         public string FindUserRoleByUserId(int userId)
         {
             ...
             try
             {
                 ...
                 {
                     conn.Open();
                     using (SqlCommand command = new SqlCommand(Constants.FindRoleByUserId, conn))
                     {
                         command.CommandType = CommandType.StoredProcedure;
                         command.Parameters.Add(new SqlParameter(Constants.UserIdParameter, userId));
                         ...
                         using (SqlDataReader reader = command.ExecuteReader())
                         {
                             ...
                             if (reader.Read())
                             {
                                 userRoleName = reader.IsDBNull(2) ? string.Empty : reader.GetString(2);
                             }
                             else
                             {
                                 // Nothing to do
